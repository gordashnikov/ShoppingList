...
import com.rustyrobot.shoppinglist.databinding.ActivityShopItemBinding
import com.rustyrobot.shoppinglist.domain.ShopItem.Companion.UNDEFINED_ID
import com.rustyrobot.shoppinglist.presentation.ShopItemFragment.Companion.newInstanceAddItem
import com.rustyrobot.shoppinglist.presentation.ShopItemFragment.Companion.newInstanceEditItem
...
class ShopItemActivity : AppCompatActivity() {
    ...
    private fun parseIntent() {
        if (!intent.hasExtra(EXTRA_SCREEN_MODE)) throw RuntimeException("Parameter screen mode is absent")
        val mode = intent.getStringExtra(EXTRA_SCREEN_MODE)
        if (mode.toString() != MODE_EDIT && mode.toString() != MODE_ADD) throw RuntimeException("Unknown screen mode: $mode")
